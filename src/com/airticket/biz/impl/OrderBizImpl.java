	...
	public boolean saveByOrder(Order order) {
		...
		if(null!=order){
			try {
				...
				if(null!=orderId&&null!=orderId.getKey()&&orderId.getKey().equals(StaticData.EMPTY)){
					UntreatedOrder untreatedOrder = (UntreatedOrder)order;
					untreatedOrder.setOrderid(orderId.getKey());
					Serializable id = orderDao.saveByOrder(untreatedOrder).toString();
					if(null!=id){
						isok  =true;
					}
				}
